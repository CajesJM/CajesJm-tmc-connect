    ...
    const fetchStats = async () => {
      try {
        ...
        allEventsSnap.docs.forEach((d) => {
          const data = d.data();
          const s = data.status;
          const isApproved = s === 'approved' || s === undefined || s === null || s === '';
        ...
        const activeEvents = allEventsSnap.docs.filter((d) => {
          ...
          const s = data.status;
          const isApproved = s === 'approved' || s === undefined || s === null || s === '';
          const eventDate = data.date?.toDate?.() ?? (data.date ? new Date(data.date) : null);
          return isApproved && eventDate && eventDate > now;
        }).length;

        const annSnap = await getDocs(collection(db, 'updates'));
    ...
    <View style={styles.container}>
      ...
      >
        <View style={styles.headerContent}>
          <View>
            <Text style={[styles.greetingText, { color: isDark ? 'rgba(148,163,184,0.85)' : 'rgba(255,255,255,0.75)' }]}>
              My Profile
            </Text>
            <Text style={styles.userName}>{displayName}</Text>
            <Text style={[styles.roleText, { color: isDark ? 'rgba(148,163,184,0.6)' : 'rgba(255,255,255,0.65)' }]}>
        ...
        <View style={styles.dateSection}>
          <View style={styles.dateContainer}>
            <Text style={[styles.dateText, { color: isDark ? colors.sidebar?.text?.muted || '#94a3b8' : '#ffffff' }]}>
        ...
        <View style={styles.identityCard}>
          ...
          <View style={styles.statsRow}>
            {[
              ...
              {
                value: loadingStats ? '—' : (stats.totalAnnouncements + stats.totalEvents),
                label: 'Posts',
              },
            ].map((s, i, arr) => (
              <React.Fragment key={s.label}>
                <View style={styles.statCell}>
                  {loadingStats ? (
                    <ActivityIndicator size="small" color={colors.accent?.primary || '#0ea5e9'} />
                  ) : (
        ...
        <View style={styles.sectionCard}>
          ...
          <View style={styles.analyticsGrid}>
            <View style={[styles.highlightCard, styles.highlightCardRow]}>
              <View style={styles.highlightCardLeft}>
                <View style={styles.highlightIconRow}>
                  <View style={[styles.highlightIconBox, { backgroundColor: isDark ? 'rgba(16,185,129,0.15)' : '#d1fae5' }]}>
                    <Feather name="check-square" size={16} color="#10b981" />
                  </View>
                  <View style={[styles.highlightTrend, { backgroundColor: isDark ? 'rgba(16,185,129,0.15)' : '#d1fae5' }]}>
                    <Feather name="users" size={9} color="#10b981" />
                    <Text style={[styles.highlightTrendText, { color: '#10b981' }]}>total</Text>
                  </View>
                </View>
                {loadingStats
                  ? <ActivityIndicator color="#10b981" style={{ marginTop: 8 }} />
                  : <Text style={styles.highlightValue}>{stats.totalAttendance}</Text>
                }
            ...
            <View style={[styles.highlightCard, styles.highlightCardRow]}>
              <View style={styles.highlightCardLeft}>
                <View style={styles.highlightIconRow}>
                  <View style={[styles.highlightIconBox, { backgroundColor: isDark ? 'rgba(245,158,11,0.15)' : '#fef3c7' }]}>
                    <Feather name="bell" size={16} color="#f59e0b" />
                  </View>
                  <View style={[styles.highlightTrend, { backgroundColor: isDark ? 'rgba(245,158,11,0.15)' : '#fef3c7' }]}>
                    <Feather name="clock" size={9} color="#f59e0b" />
                    <Text style={[styles.highlightTrendText, { color: '#f59e0b' }]}>
                      {loadingStats ? '—' : `${stats.pendingEvents + stats.pendingAnnouncements} pend.`}
                    </Text>
                  </View>
                </View>
                {loadingStats
                  ? <ActivityIndicator color="#f59e0b" style={{ marginTop: 8 }} />
                  : <Text style={styles.highlightValue}>{stats.totalEvents + stats.totalAnnouncements}</Text>
                }
            ...
            <View style={styles.statusRow}>
              {[
                { label: 'Approved', value: stats.combinedApproved, color: '#10b981', bg: isDark ? 'rgba(16,185,129,0.12)' : '#d1fae5' },
                { label: 'Pending', value: stats.combinedPending, color: '#f59e0b', bg: isDark ? 'rgba(245,158,11,0.12)' : '#fef3c7' },
                { label: 'Rejected', value: stats.combinedRejected, color: '#ef4444', bg: isDark ? 'rgba(239,68,68,0.12)' : '#fee2e2' },
              ].map((s) => (
                <View key={s.label} style={[styles.statusCell, { backgroundColor: s.bg }]}>
                  {loadingStats
                    ? <ActivityIndicator size="small" color={s.color} />
                    : <Text style={[styles.statusCellValue, { color: s.color }]}>{s.value}</Text>
                  }
                  <Text style={[styles.statusCellLabel, { color: s.color }]}>{s.label}</Text>
                </View>
              ))}
            </View>
            ...
            <View style={styles.chartBlock}>
              ...
              {[
                { label: 'Students', value: stats.totalStudents, color: '#0ea5e9' },
                { label: 'Assistant Admins', value: stats.assistantAdmins, color: '#8b5cf6' },
                { label: 'Main Admins', value: stats.mainAdmins, color: '#f59e0b' },
              ].map((row) => {
                  ...
                  <View key={row.label} style={styles.barRow}>
                    <View style={styles.barMeta}>
                      <View style={styles.barMetaLeft}>
                        <View style={[styles.barDot, { backgroundColor: row.color }]} />
            ...
            {(() => {
              const rate = stats.totalStudents > 0 && stats.totalAttendance > 0
                ? Math.min(100, Math.round((stats.totalAttendance / (stats.totalStudents * Math.max(stats.approvedEvents, 1))) * 100))
                : 0;
              const ringColor = rate >= 70 ? '#10b981' : rate >= 40 ? '#f59e0b' : '#ef4444';
              return (
                <View style={styles.rateBlock}>
                  <View style={[styles.rateRing, { borderColor: ringColor, backgroundColor: isDark ? `${ringColor}18` : `${ringColor}12` }]}>
                    {loadingStats
                      ? <ActivityIndicator size="small" color={ringColor} />
                      : <Text style={[styles.rateRingValue, { color: ringColor }]}>{rate}%</Text>
                    }
                  </View>
                  <View style={styles.rateInfo}>
                    ...
                    <View style={styles.rateBarTrack}>
                      <View style={[styles.rateBarFill, { width: `${rate}%`, backgroundColor: ringColor }]} />
                    </View>
                  </View>
                </View>
              );
