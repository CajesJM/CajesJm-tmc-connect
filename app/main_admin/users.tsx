...
export default function UserManagement() {
  ...
  const [activeFilter, setActiveFilter] = useState<
    'all' | 'main_admin' | 'assistant_admin' | 'student'
  >('all')
  ...
  const filterUsersByRole = (
    usersList: User[],
    filter: 'all' | 'main_admin' | 'assistant_admin' | 'student'
  ) => {
  ...
  const validateCreateUser = (): { isValid: boolean; errorMessage: string } => {
    ...
    if (newUser.role === 'student' && !newUser.surname?.trim()) {
      return {
        isValid: false,
        errorMessage: 'Surname is required for students.',
      }
    }
    ...
    if (newUser.role === 'student') {
      ...
      if (!newUser.course?.trim()) {
        return {
          isValid: false,
          errorMessage: 'Please enter the course (e.g., BSIT, BSCS).',
        }
      }
    ...
    if (newUser.role !== 'student') {
      if (!newUser.password?.trim()) {
        return {
          isValid: false,
          errorMessage: 'Please enter a password for the admin account.',
        }
      }
    ...
    try {
      ...
      if (newUser.role === 'student' && newUser.studentID) {
        ...
        const studentIdQuery = query(
          usersCollection,
          where('studentID', '==', newUser.studentID.trim())
        )
        ...
        <View style={styles.searchResultHeader}>
          ...
          <Text
            style={[
              ...
              isMobile && styles.searchResultEmailMobile,
            ]}
          >
            {item.email}
          </Text>
              ...
              >
                ...
                <Text
                  style={[
                    styles.searchResultEmail,
                    isMobile && styles.searchResultEmailMobile,
                    { color: '#10b981', fontWeight: '600', marginLeft: 4 },
                  ]}
                >
        ...
        <View style={styles.searchResultFooter}>
          <View style={styles.searchResultRole}>
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              ...
              <Text
                style={[
                  styles.searchResultRoleText,
                  isMobile && styles.searchResultRoleTextMobile,
                  { marginLeft: 4 },
                ]}
              >
                {item.username}
    ...
    return (
      <View>
        ...
        <View style={styles.modernDetailRow}>
          ...
          <Text style={styles.modernDetailRowText}>{selected.name}</Text>
        </View>
        ...
        <View style={styles.modernDetailRow}>
          ...
          <Text style={styles.modernDetailRowText}>{selected.email}</Text>
        </View>

        <View style={styles.modernDetailRow}>
          <Feather name='at-sign' size={16} color={colors.accent.primary} />
          <Text style={styles.modernDetailRowText}>@{selected.username}</Text>
        </View>
                    ...
                    {(
                      ...
                      <TouchableOpacity
                        ...
                        style={[
                          ...
                          (isEdit ? selectedUser?.role : newUser.role) ===
                            role && styles.modernRoleOptionSelected,
                        ]}
                        onPress={() => {
                      ...
                      >
                        <Text
                          style={[
                            ...
                            (isEdit ? selectedUser?.role : newUser.role) ===
                              role && styles.modernRoleOptionTextSelected,
                          ]}
                        >
      ...
      <View style={[styles.mainContent, isMobile && styles.mainContentMobile]}>
        ...
        <View style={[styles.leftGrid, isMobile && styles.leftGridMobile]}>
          ...
          >
            ...
            >
              <Text
                style={[styles.userCount, isMobile && styles.userCountMobile]}
              >
          ...
          <View style={styles.resultsInfo}>
            <Text style={styles.resultsText}>
              {filterUsersByRole(users, activeFilter).length} user
              {filterUsersByRole(users, activeFilter).length !== 1 ? 's' : ''}
              {activeFilter !== 'all' &&
                ` from ${activeFilter.replace('_', ' ')}`}
            </Text>
          </View>
