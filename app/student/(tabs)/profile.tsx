...
export default function StudentProfile() {
  ...
  const renderHelpModal = () => {
    const faqs = [
      ...
      },
      {
        ...
        answer:
          'When you miss an event, it will appear in your "Missed" tab. The admin may issue penalties for unexcused absences. You can view your penalties in the "My Penalties" section.',
      ...
      },
      {
        ...
        answer:
          'Yes! Tap on your profile picture in the Profile section, then choose to either take a new photo or select one from your gallery.',
        icon: 'camera',
      },
      {
        id: '5',
        ...
        answer:
          "Make sure you have notifications enabled in your device settings. You'll receive alerts for new events, announcements, attendance reminders, and penalty updates.",
        icon: 'bell',
      },
      {
        id: '6',
        ...
        answer:
          'Penalties vary depending on the event importance and frequency of absences. Check your "My Penalties" section for specific details about any pending penalties.',
        icon: 'alert-circle',
      },
      {
        id: '7',
        ...
        answer:
          'Course information can only be updated by administrators. Contact your department or admin if you need to make changes to your course details.',
        icon: 'school',
      },
      {
        id: '8',
        ...
        answer:
          'Yes, we take data security seriously. All your personal information and attendance records are securely stored and only accessible to authorized personnel.',
        icon: 'shield-account',
      },
    ]
          ...
          <View style={[styles.aboutModal, { maxHeight: '90%', padding: 0 }]}>
            ...
            >
                  ...
                  >
                    ...
                    <Text
                      style={[
                        ...
                        { color: colors.sidebar.text.secondary, fontSize: 12 },
                      ]}
                    >
                      {channel.contact}
                    </Text>
              ...
              >
                ...
                >
                  Help us improve TMC Connect by reporting any issues you
                  encounter. Our team will investigate and fix it as soon as
                  possible.
                </Text>
                <TouchableOpacity
                  style={[
  ...
  const calculateMonthlyAttendance = useCallback(async () => {
    try {
      ...
      querySnapshot.docs.forEach((docSnapshot) => {
        ...
        const studentAttendance = attendees.find(
          (attendee: AttendanceRecord) =>
            attendee.studentID === studentID.toString()
        )
        const eventMonthIndex = months.findIndex(
          (month) => eventDate >= month.startDate && eventDate <= month.endDate
        )
    ...
    try {
      ...
      for (const docSnapshot of querySnapshot.docs) {
        ...
        const eventInfo: StudentEvent = {
          ...
          scannedAt:
            studentAttendance?.scannedAt || studentAttendance?.timestamp,
        }
      ...
      (snapshot) => {
        ...
        if (pendingPenalties.length > 0) {
          ...
          if (newest && newest.createdAt) {
            const newestMillis =
              newest.createdAt.toMillis?.() || newest.createdAt.getTime()
  ...
  const renderFullAttendedModal = () => (
    <Modal
      visible={showFullAttendedModal}
      transparent={true}
      animationType='slide'
      onRequestClose={() => setShowFullAttendedModal(false)}
    >
      <View style={styles.modalOverlay}>
        <View style={[styles.aboutModal, { maxHeight: '90%' }]}>
          <View style={styles.modalHeader}>
            <Text style={[styles.modalTitle, { color: colors.text }]}>
              All Attended Events ({attendedEvents.length})
            </Text>
            <TouchableOpacity onPress={() => setShowFullAttendedModal(false)}>
              <Icon name='close' size={24} color={colors.text} />
            </TouchableOpacity>
          </View>
          <ScrollView showsVerticalScrollIndicator={false}>
            <View style={{ padding: 16 }}>
              {attendedEvents.map((event) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.eventItem,
                    styles.attendedEventItem,
                    { marginBottom: 12 },
                  ]}
                  onPress={() => {
                    setShowFullAttendedModal(false)
                    setSelectedEventDetail(event)
                    setShowEventDetailModal(true)
                  }}
                >
                  <View
                    style={[styles.eventIcon, { backgroundColor: '#10B98115' }]}
                  >
  ...
  const renderFullMissedModal = () => (
    <Modal
      visible={showFullMissedModal}
      transparent={true}
      animationType='slide'
      onRequestClose={() => setShowFullMissedModal(false)}
    >
      <View style={styles.modalOverlay}>
        <View style={[styles.aboutModal, { maxHeight: '90%' }]}>
          <View style={styles.modalHeader}>
            <Text style={[styles.modalTitle, { color: colors.text }]}>
              All Missed Events ({missedEvents.length})
            </Text>
            <TouchableOpacity onPress={() => setShowFullMissedModal(false)}>
              <Icon name='close' size={24} color={colors.text} />
            </TouchableOpacity>
          </View>
          <ScrollView showsVerticalScrollIndicator={false}>
            <View style={{ padding: 16 }}>
              {missedEvents.map((event) => {
                ...
                const isCompleted =
                  penalty?.status === 'paid' || penalty?.status === 'completed'
                return (
                  <TouchableOpacity
                    key={event.id}
                    style={[styles.eventItem, { marginBottom: 12 }]}
                    onPress={() => {
                      setShowFullMissedModal(false)
                      setSelectedEventDetail(event)
                      setShowEventDetailModal(true)
                    }}
                  >
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.aboutModal, { maxHeight: '90%', padding: 0 }]}>
            ...
            >
              ...
              <TouchableOpacity
                onPress={() => setShowAttendanceReportModal(false)}
              >
                <Icon name='close' size={24} color={colors.text} />
              </TouchableOpacity>
            </View>
                    ...
                    {selectedSlice !== null && (
                      <Animated.View
                        style={[
                          ...
                          {
                            backgroundColor: isDark
                              ? `${pieData[selectedSlice].color}20`
                              : `${pieData[selectedSlice].color}10`,
                            opacity: chartAnim,
                            transform: [
                              {
                                translateY: chartAnim.interpolate({
                      ...
                      >
                        <Icon
                          name={pieData[selectedSlice].icon as any}
                          size={20}
                          color={pieData[selectedSlice].color}
                        />
                        <Text
                          style={[
                            styles.sliceDetailsText,
                            { color: pieData[selectedSlice].color },
                          ]}
                        >
  ...
  const renderEventDetailModal = () => {
    ...
    const penalty = penaltyMap[selectedEventDetail.id]
    const isAttended = attendedEvents.some(
      (e) => e.id === selectedEventDetail.id
    )
              ...
              <View style={{ padding: 16 }}>
                ...
                >
                  <Icon
                    ...
                    color={colors.sidebar.text.secondary}
                  />
                  <Text style={[styles.eventDate, { marginLeft: 8 }]}>
                    {formatDate(selectedEventDetail.date)}
                  </Text>
                </View>
                ...
                {isAttended && selectedEventDetail.scannedAt && (
                  <View
                    style={{
                  ...
                  >
                    ...
                    >
                      Attended at{' '}
                      {new Date(selectedEventDetail.scannedAt).toLocaleString()}
                    </Text>
                  </View>
                )}

                {!isAttended && selectedEventDetail.attendanceDeadline && (
                  <View
                    style={{
                  ...
                  >
                    <Text style={{ color: '#EF4444', fontWeight: '600' }}>
                      Deadline:{' '}
                      {formatDate(selectedEventDetail.attendanceDeadline)}
                    </Text>
                  </View>
                )}
                ...
                {penalty && (
                  ...
                  >
                    ...
                    <Text style={{ color: colors.sidebar.text.secondary }}>
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.heroCard}>
          <View style={styles.heroContent}>
            <View style={styles.avatarSection}>
              ...
              <Text style={styles.avatarLabel}>Student</Text>
            </View>
            ...
            <View style={styles.identitySection}>
              ...
              <Text style={styles.courseInfo}>
                {userCourse} • {userYearLevel} • Block {userBlock}
              </Text>
            </View>
          </View>
        ...
        <View style={styles.sectionCard}>
          ...
          {loading ? (
            ...
            attendedEvents.length === 0 ? (
              ...
              <View style={styles.eventsList}>
                {attendedEvents.slice(0, 5).map((event) => (
                  <TouchableOpacity
                    key={event.id}
                    style={[styles.eventItem, styles.attendedEventItem]}
                    onPress={() => {
                      setSelectedEventDetail(event)
                      setShowEventDetailModal(true)
                    }}
                    ...
                    <View style={styles.eventDetails}>
                      ...
                      {event.scannedAt && (
                        ...
                        >
                          <Icon name='clock-check' size={12} color='#10B981' />
                          <Text
                            style={[styles.scannedAtText, { marginLeft: 4 }]}
                          >
                            Scanned at{' '}
                            {new Date(event.scannedAt).toLocaleTimeString()}
                          </Text>
                ...
                {attendedEvents.length > 5 && (
                  <TouchableOpacity
                    ...
                    onPress={() => setShowFullAttendedModal(true)}
                  >
                    <Text style={styles.viewAllButtonText}>
                      View all {attendedEvents.length} attended events
                    </Text>
                    <Icon
                      name='chevron-right'
                      size={16}
            ...
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptyStateText}>
                Great! You haven't missed any events.
              </Text>
            </View>
          ) : (
            <View style={styles.eventsList}>
              ...
              {missedEvents.length > 5 && (
                <TouchableOpacity
                  style={styles.viewAllButton}
                  onPress={() => setShowFullMissedModal(true)}
                >
                  <Text style={styles.viewAllButtonText}>
                    View all {missedEvents.length} missed events
                  </Text>
                  <Icon
                    name='chevron-right'
                    size={16}
          ...
          <View style={styles.menuList}>
            ...
            >
              <View style={styles.menuItemLeft}>
                <Icon name='book-open-variant' size={18} color='#3B82F6' />
            ...
            >
              <View style={styles.menuItemLeft}>
                <Icon name='alert-outline' size={18} color='#EF4444' />
        ...
        <View style={styles.footer}>
          ...
          <Text style={styles.footerSub}>Student Panel</Text>
        </View>
      </ScrollView>
