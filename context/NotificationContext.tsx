...
export const NotificationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const computeCounts = (notifications: Notification[]): Record<TabKey, number> => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = notificationService.listenForNotifications(user.uid, (notifications) => {
  ...
  const clearUnread = async (tab: TabKey) => {
    ...
    // Listener will auto-update counts
  };
  ...
  return (
    <NotificationContext.Provider value={{ unreadCounts, clearUnread, markAllAsRead }}>
      {children}
