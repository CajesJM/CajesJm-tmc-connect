...
export const usePushNotifications = () => {
  ...
  useEffect(() => {
    ...
    const setupNotifications = async () => {
      ...
      // Register for push token (only on physical device)
      if (Device.isDevice) {
        ...
        if (finalStatus === 'granted') {
          ...
          try {
            const pushTokenData = await Notifications.default.getExpoPushTokenAsync({ projectId });
      ...
      // Set up listeners
      notificationListener.current = Notifications.default.addNotificationReceivedListener((notification) => {
        if (isMounted) setNotification(notification);
      });

      responseListener.current = Notifications.default.addNotificationResponseReceivedListener((response) => {
        const data = response.notification.request.content.data as { screen?: string; params?: Record<string, string> };
        if (data?.screen) {
